...
require_once __DIR__ . "/../../Model/Admin/AccountModel.php";
$model = new AccountModel();

$adminUsername = $_SESSION['user_email'] ?? '';
$admin = $model->getAdminByUsername($adminUsername);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
    ...
    if ($username !== $admin['Admin_Username']) {
        ...
        if ($model->updateAccount($admin['Admin_ID'], $fname, $lname, $username)) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['verify_otp'])) {
    ...
    if (isset($_SESSION['otp'], $_SESSION['pending_email']) &&
        ...
        if ($model->updateAccount($admin['Admin_ID'], $admin['Admin_Fname'], $admin['Admin_Lname'], $_SESSION['pending_email'])) {
            $message = "✅ Email changed successfully.";
            $_SESSION['user_email'] = $_SESSION['pending_email'];
            $admin = $model->getAdminByUsername($_SESSION['pending_email']);
        } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['change_password'])) {
    ...
    $checkRow = $model->getPassword($admin['Admin_ID']);
    $currentHash = $checkRow['Admin_HashedPassword'] ?? null;
    ...
    if (!$currentHash || !password_verify($oldPassword, $currentHash)) {
        ...
        if ($model->updatePassword($admin['Admin_ID'], $newPassword)) {
...
require_once __DIR__ . "/../../View/Admin/AccountView.php";
renderAccountView($admin, $message, $error, $modalError, $modalMessage);
?>